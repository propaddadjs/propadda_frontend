// src/components/BuyerLayout.tsx
...
const Breadcrumbs: React.FC = () => {
  ...
  // Create an array of path segments from the URL (e.g., ['Buyer', 'listings', 'all'])
  const pathnames = location.pathname.split('/').filter((x) => x);

  // If we are on the main dashboard page, show the original title
  if (pathnames.length <= 1) {
    return <div className="text-lg font-semibold tracking-tight">Buyer Dashboard</div>;
  }

  return (
    <div className="flex items-center text-sm">
      <Link to="/Buyer" className="text-gray-500 hover:text-gray-800">Home</Link>
      {/* We map over the segments, skipping the first one ('Buyer') */}
...
const BuyerLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      {/* Sidebar */}
      <BuyerSidebar collapsed={sidebarCollapsed} />
        ...
        <header className="bg-white border-b px-4 sm:px-6 py-3 flex items-center justify-between sticky top-0 z-30">
          ...
          <div className="flex items-center gap-4">
            ...
            <div ref={profileRef} className="relative">
              ...
              >
                {/* Here we put user's name and email */}
