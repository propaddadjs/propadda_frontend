...
//   const editListingPath = `/agent/listings/edit/${p.category ?? "Residential"}/${p.listingId}`; // Assuming an edit route
  ...
  return (
    <div
      ...
      className="rounded-xl shadow-md flex flex-col border-2 border-gray-200 bg-yellow-100 overflow-hidden transition delay-150 duration-300 ease-in-out hover: hover:scale-105 hover:shadow-lg hover:border-yellow-500"
    >
...
/* All Listings Page */
const AgentSoldListings: React.FC = () => {
  ...
  const fetchListings = async () => {
    ...
    try {
      ...
      const resp = await axios.get<AgentAllPropertiesResponse>(
        `${API_BASE_URL}/agent/soldPropertiesByAgent/${AGENT_ID}`
      );
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-8">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Sold Listings</h2>
      ...
      {loading ? (
        ...
        <div className="p-8 text-center bg-white rounded-xl shadow-sm text-gray-600">
          You have no properties sold yet.
          <Link to="/agent/listings/active" className="text-orange-500 hover:underline block mt-2">
            Click here to view your active listings.
          </Link>
        </div>
      ) : (
...
export default AgentSoldListings;