        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="text-right">
            ...
            <div className="mt-2 flex flex-col items-end gap-2">
              {p.vip && (
                <div className="text-xs font-semibold text-yellow-800 bg-yellow-200 px-2 py-0.5 rounded flex items-center gap-1">
      ...
      <div className="flex flex-col justify-between items-end">
        <div className="flex flex-col gap-2">
          <Link
            ...
            className="px-3 py-1 text-white text-center rounded text-sm bg-emerald-600 transition delay-150 duration-300 ease-in-out hover: hover:scale-105 hover:bg-emerald-700 flex items-center justify-center gap-1"
          >
...
const VipListings: React.FC = () => {
  ...
  const fetch = async (filters: SidebarFilters | null = null) => {
    ...
    try {
      ...
      console.error("fetch vip properties error:", err);
      setRawData([]);
      setPageSlice([]);
      setTotalPages(0);
    } finally {
    ...
    <div className="flex">
      ...
      <main className="flex-1 p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-semibold">VIP Listings</h2>
        ...
        <div className="space-y-4">
          {pageSlice.length === 0 && !loading && <div className="text-gray-600">No VIP Listings found.</div>}

          {[...pageSlice]
            .sort((a, b) => (b?.listingId ?? 0) - (a?.listingId ?? 0)) // DESC by listingId
            .map((p) => (
              <Card key={p.listingId} p={p} onUnmarkVip={unmarkVip} />
            ))}
        </div>
